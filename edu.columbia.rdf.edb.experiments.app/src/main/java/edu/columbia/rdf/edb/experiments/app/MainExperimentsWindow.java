...
public class MainExperimentsWindow extends ModernRibbonWindow implements ModernClickListener {
	...
	private ExperimentSummaryPanel mExperimentSummaryPanel = 
			new ExperimentSummaryPanel(mSampleSelectionModel);
	...
	/**
	 * Display an error message to the user that no
	...
	public MainExperimentsWindow(GuiAppInfo appInfo) {
		...
		mAnnotationPanel = new SampleViewPanel(this,
				mViewModel,
				mLayoutViewModel,
				mSampleModel,
		...
		try {
			mSampleFolderPanel = 
					new SearchFolderTreePanel(mSampleFolderModel);
	...
	public final void createRibbon() {
		...
		//button = new QuickAccessButton(UIResources.getInstance().loadIcon(DownloadVectorIcon.class, 16));
		...
		getRibbon().getToolbar("View").add(new RibbonWideLayoutSection(getRibbon(),
				"Layout",
				mLayoutViewModel));
		...
		mSearchExperimentTitleButton.setToolTip(new ModernToolTip("Search by Experiment Title", 
				"Search experiment titles by keywords."), getRibbon());

		Ui.setSize(mSearchExperimentTitleButton, new Dimension(60, Ribbon.LARGE_BUTTON_HEIGHT));
		buttonContainer.add(mSearchExperimentTitleButton);

		mSearchSampleTitleButton.addClickListener(this);
		mSearchSampleTitleButton.setToolTip(new ModernToolTip("Search by Sample Title", 
		...
		Ui.setSize(mGeoAccessionButton, new Dimension(120, ModernWidget.WIDGET_HEIGHT));
		mGeoAccessionButton.setToolTip(new ModernToolTip("Search by GEO Accession", 
				"Search for samples with a GEO accession matching search terms."), getRibbon());
		mGeoAccessionButton.addClickListener(this);
		vPanel.add(mGeoAccessionButton);

		ModernScrollPopupMenu searchCriteriaPopup = 
				new ExperimentsSearchCriteriaPopup(this);
		...
		mSearchWithinButton.setToolTip(new ModernToolTip("Search In Current Folder Only", 
				"Restrict searches to the current folder."), 
				getRibbon());
	...
	public void init() {
		...
		if (SettingsService.getInstance().getAsBool("edb.experiments.default-search")) {
			// perform an initial search with no filter to return all experiments so that the
	 ...
	 */
	private void reloadCache() throws SAXException, IOException, ParserConfigurationException, KeyManagementException, NoSuchAlgorithmException, FontFormatException, ClassNotFoundException, InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
		...
		status = ModernMessageDialog.createDialog(this, 
				getAppInfo().getName(), 
				MessageDialogType.INFORMATION_YES_NO,
				"Reloading the cache requires " + getAppInfo().getName() + " to be restarted.", 
	...
	/**
	 * Adjusts the height of the search bar depending on how many
	 * criteria the user has added.
	 */
	private void resizeSeachPanel() {
		//annotationPanel.setDividerLocation(SEARCH_ROW_OFFSET + SEARCH_ROW_HEIGHT * Math.max(1, searchPanel.getSearch().size()));
	...
	private void downloadFiles() throws IOException {
		/*
		switch (mTabsModel.getSelectedIndex()) {
		case 0:
		case 2:
			downloadSampleFiles();
			break;
		case 1:
			dataTab.downloadFiles();
			break;
		}
		 */
	}
	...
	private void downloadSampleFiles() throws IOException, ParseException {
		...
		boolean cel = dialog.getDownloadCel();
		boolean chp = dialog.getDownloadCHP();
		boolean mas5 = dialog.getDownloadMAS5();
		boolean rma = dialog.getDownloadRMA();
		...
		for (FileDescriptor file : repository.getSampleFiles(samples)) {
			...
			if (cel && file.getName().toLowerCase().endsWith("cel")) {
				download = true;
			}

			if (chp && file.getName().toLowerCase().endsWith("chp")) {
				download = true;
			}
	...
	*/
	...
	/*
	private void showSDRF() throws IOException {
		List<Sample> samples = annotationPanel.getSelectedSamples();
...
//				mUserGroupsModel.getGroups());
	...
	/**
	 * We take control of the tab changing so we can adjust
	 * the ribbon and other features as well as the tab.
	 * 
	 */
	/*
	private void changeTab() {
		if (mTabsModel.getSelectedTab() == null) {
			return;
		}
		...
		if (mTabsModel.getSelectedTab().getName().equals("Files")) {
			...
			//getRibbon().changeView("annotation");
	...
	private void addCategoryPane() {
		if (mContentPane.getModel().contains("Categories")) {
			return;
		}
		...
		mContentPane.getModel().addLeft(new SizableContentPane("Categories", 
				new CloseableHTab("Categories", mCategoryTreePanel, mContentPane.getModel()), 500, 300, 600));
	}
	 */
	 ...
	 * @throws UnsupportedLookAndFeelException the unsupported look and feel exception
	 */
	private void createInfoTable() throws ClassNotFoundException, InstantiationException, IllegalAccessException, FontFormatException, IOException, UnsupportedLookAndFeelException {
		...
		Map<DataViewField, Integer> indexMap = 
				new HashMap<DataViewField, Integer>();
