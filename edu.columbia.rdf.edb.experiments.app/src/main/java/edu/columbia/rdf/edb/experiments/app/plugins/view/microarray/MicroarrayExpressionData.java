...
public class MicroarrayExpressionData {
	...
	private static final Logger LOG = 
			LoggerFactory.getLogger(MicroarrayExpressionData.class);
	...
	public void showTables(ModernWindow parent,
		...
		Map<String, Set<Sample>> samplesGroupedByArray =
				Sample.sortByArrayDesign(samples);
	...
	public void download(Frame parent,
		...
		Path allSamplesFile = 
				pasteFiles(sampleFiles, columns, columnAnnotations, statusModel);
	...
	private Path pasteFiles(Map<Sample, Path> sampleFileMap,
		...
		//System.err.println("Writing to " + tempFile1 + " " + commonProbesList.size() +  " " + samples.size());
		...
		for (Entry<Sample, File> entry : sampleFileMap.entrySet()) {
			readers.put(entry.getKey(), new BufferedReader(new FileReader(entry.getValue())));
		}

		List<String> tokens;

		for (Entry<Sample, BufferedReader> entry : readers.entrySet()) {
			...
			try {
				...
				// don't need the id column as this is duplicated in
				// every expression file
				...
				for (int c = 0; c < tokens.size(); ++c) {
					...
					if (columnAnnotations != null) {
						...
						for (DataViewField a : columnAnnotations) {
							// TODO check if getName correct
							v.add(sample.getSection(a.getSection().getType()).getField(a.getName()));
						}

						if (v.size() > 0) {
							h += " (" + TextUtils.join(v, TextUtils.COMMA_DELIMITER) + ")";
						}
					}
				...
				while ((line = reader.readLine()) != null) {
					...
					if (!probeText.containsKey(probe)) {
						continue;
					}
					...
					// store the probe
					probeText.put(probe, TextUtils.join(printColumns, TextUtils.TAB_DELIMITER));
				}
			} finally {
				reader.close();
			}

			// for this particular file we can now write it out
			// by concatenating it with the temp file

			// first the header

			System.err.println("header " + header.toString() + " " + tempFile1 + " " + commonProbes.size());

			reader = new BufferedReader(new FileReader(tempFile1));
			...
			try {
				...
				// now the probes. Since the temp file is in order
				// of common probes, we simply write it out in order

				for (String probe : commonProbes) {
					//System.err.println("probe " + probe);

					writer.write(reader.readLine());
					writer.write(TextUtils.TAB_DELIMITER);
					writer.write(probeText.get(probe));
					writer.newLine();
				}
			} finally {
				reader.close();
			...
			// now move temp2 to temp1 and begin again
			boolean success = tempFile2.renameTo(tempFile1);

			System.err.println("rename " + tempFile2 + " to " + tempFile1 + " " + success);
		}
		...
		try {
			...
			writer.write(DataMatrix.EST_VERSION_1);
			writer.newLine();

			//number of rows
			writer.write(DataMatrix.EST_ROWS);
			writer.write(TextUtils.TAB_DELIMITER);
			writer.write(Integer.toString(commonProbes.size()));
			writer.newLine();

			//columns
			writer.write(DataMatrix.EST_COLUMNS);
			writer.write(TextUtils.TAB_DELIMITER);
			writer.write(Integer.toString(header.size()));
			writer.newLine();
	...
	private final FileDescriptor getRemoteExpressionFile(Sample sample,
			MicroarrayNormalizationType normalisationType) throws IOException, ParseException {
