...
public class SampleSortRibbonSection extends RibbonSection implements ModernClickListener {
	...
	private ModernCheckButton sortDescendButton = 
			new ModernCheckButton(UIService.getInstance().loadIcon("sort_descend", 16));
	
	/** The sort ascend button. */
	private ModernCheckButton sortAscendButton = 
			new ModernCheckButton(UIService.getInstance().loadIcon("sort_ascend", 16));
	
	/** The m button sort map. */
	private Map<String, ModernCheckBox> mButtonSortMap = 
