 ...
 * for search through experiments and samples.
 *
...
public class ExperimentsSearchCriteriaPopup extends SearchCriteriaPopup {
	...
	//public static final Dimension MENU_ITEM_SIZE = new Dimension(400, Resources.ICON_SIZE_48);
	...
	public ExperimentsSearchCriteriaPopup(ModernWindow parent) {
		...
		for (SearchCategoryGroup categoryGroup : SearchCategoryService.getInstance()) {
			...
			//item = new ModernMenuItemGridBlock(categoryGroup.getName(), MENU_WIDTH, COLUMNS);
			...
			for (SearchCategory category : categoryGroup) {
				...
				//		Resources.getInstance().loadIcon("search_criterion", Resources.ICON_SIZE_32));
		...
		for (SearchFieldGroup group : SearchFieldGroupsService.getInstance()) {
			...
			for (SearchField criterion : group) {
				c.put(criterion.getDisplayName(), criterion);
			}
