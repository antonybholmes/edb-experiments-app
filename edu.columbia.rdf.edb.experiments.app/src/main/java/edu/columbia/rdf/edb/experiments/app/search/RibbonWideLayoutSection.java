...
public class RibbonWideLayoutSection extends RibbonSection implements ModernClickListener {
	...
	private ModernCheckButton mVerticalButton =
			new RibbonLargeRadioButton("Vertical", UIService.getInstance().loadIcon(ViewVerticalVectorIcon.class, 32));
	
	/** The m wide button. */
	private ModernCheckButton mWideButton =
			new RibbonLargeRadioButton("Wide", UIService.getInstance().loadIcon(ViewWideVectorIcon.class, 32));
	
	//private ModernCheckButton listButton =
	//		new RibbonLargeRadioButton2("List", UIResources.getInstance().loadIcon("view_list", UIResources.ICON_SIZE_16));

	//private ModernCheckButton tilesButton =
	//		new ModernCheckButton("Tiles", UIResources.getInstance().loadIcon("view_tiles", UIResources.ICON_SIZE_16));
	
	//private ModernCheckButton detailsButton =
	//		new ModernCheckButton("Details", UIResources.getInstance().loadIcon("view_details", UIResources.ICON_SIZE_16));
	 ...
	 */
	public RibbonWideLayoutSection(Ribbon ribbon,
			String name, 
			ViewModel model) {
		super(ribbon, name);
