 ...
 */
public class SearchCategoriesPanel extends VBox implements SearchCriteriaCategory, ModernClickListener, ModernClickEventProducer {
	...
	public static SearchStackOperator DEFAULT_BOOL_TERM = 
			SearchStackOperator.AND;


	//private static final int[] rows = {HEIGHT};
	//private static final int[] columns = {900}; // //{30, 70, 200, 400, HEIGHT};
	...
	/** The m panels. */
	private List<SearchCategoryPanel> mPanels =
			new ArrayList<SearchCategoryPanel>();

	/** The m remove map. */
	private Map<SearchCategoryPanel, Integer> mRemoveMap = 
			new HashMap<SearchCategoryPanel, Integer>();
	...
	/**
	 * Removes all but the first user search entry, so that
	 * from a user perspective, there is always one search
	...
	public final void removeUserSearchEntry(int index) {
		...
		for (int i = 0; i < indices.size(); ++i) {
			indices.get(i).setText((i + 1) + ".");
		}
		*/

		/*
		if (panels.size() > 0) {
			logical.get(0).setSelectedIndex(0);
			logical.get(0).setEnabled(false);
			logical.get(0).setVisible(false);
	...
	public final void addUserSearchEntry(SearchStackOperator booleanOperator,
		...
		SearchCategoryPanel panel = new SearchCategoryPanel(mParent, 
				mPanels.size(),
				booleanOperator,
				text,
				field);
		...
		logicalCombo.setVisible(getRowCount() != 0);
		logical.add(logicalCombo);
		add(logicalCombo);
		
		SearchCriteriaPopup searchCriteriaPopup;

		searchCriteriaPopup = 
				new ExperimentsSearchCriteriaPopup(parent, new Dimension(180, ModernTheme.getInstance().getClass("widget").getInt("height")), 1);

		ModernComboBox searchCriteriaComboBox = 
				new SearchCategoryComboBox(searchCriteriaPopup, field);

		//comboBox.setEditable(fieldEditable);
		//comboBox.setEnabled(fieldEditable);

		fields.add(searchCriteriaComboBox);
		add(searchCriteriaComboBox);

		//JLabel containsLabel = new ModernLabel("containing", JLabel.CENTER);
		//add(containsLabel);

		ModernComboBox searchTermsComboBox = new SearchTermsComboBox(SearchTermsService.getInstance());	
		searchTermsComboBox.setText(text);
		searchTermsComboBox.addKeyListener(this);
		searchTermsComboBox.setEditable(true);
		searchText.add(searchTermsComboBox);
		add(searchTermsComboBox);

		ModernButton removeButton = new ModernButton(UIResources.getInstance().loadIcon(CloseVectorIcon.class, 16));
		...
		removeButton.setVisible(getRowCount() > 1);
	...
	public UserSearch getSearch() {
		...
		for (int i = 0; i < mPanels.size(); ++i) {
			...
			SearchStackOperator op = 
					SearchStackOperator.parseOperator(mPanels.get(i).getLogical());
			
			search.add(UserSearchEntry.create(op,
					SearchCategoryService.getInstance().getSearchCategory(mPanels.get(i).getField()),
	...
	/**
	 * Builds a stack representation of an expression in reverse polish
	 * notation.
	 *
	 ...
	 */
	@Override
	public final Deque<SearchStackElementCategory> getSearchStack() throws Exception {
		...
		Deque<SearchStackElementCategory> stack = 
				SearchStackElementCategory.getSearchStack(search);
