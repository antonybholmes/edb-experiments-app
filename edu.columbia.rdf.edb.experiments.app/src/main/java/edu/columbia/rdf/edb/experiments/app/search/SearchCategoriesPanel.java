 ...
 */
public class SearchCategoriesPanel extends VBox
    implements SearchCriteriaCategory, ModernClickListener, ModernClickEventProducer {
  ...
  /**
   * Removes all but the first user search entry, so that from a user perspective,
   * there is always one search field available to them.
   */
  ...
  public final void addUserSearchEntry(UserSearchEntry userSearchEntry) {
    addUserSearchEntry(userSearchEntry.getOperator(), userSearchEntry.getField(), userSearchEntry.getText());
  }
   ...
   *          the text
   */
  public final void addUserSearchEntry(SearchStackOperator booleanOperator, SearchCategory field, String text) {

    SearchCategoryPanel panel = new SearchCategoryPanel(mParent, mPanels.size(), booleanOperator, text, field);
     ...
     * 
     * //comboBox.setEditable(fieldEditable); //comboBox.setEnabled(fieldEditable);
     * 
     ...
     * searchTermsComboBox.setText(text); searchTermsComboBox.addKeyListener(this);
     * searchTermsComboBox.setEditable(true); searchText.add(searchTermsComboBox);
     * add(searchTermsComboBox);
     * 
     * ModernButton removeButton = new
     * ModernButton(UIResources.getInstance().loadIcon(CloseVectorIcon.class, 16));
     * 
     ...
     * confuse them with a blank display removeButton.setVisible(getRowCount() > 1);
     * 
  ...
  public UserSearch getSearch() {
    ...
    for (int i = 0; i < mPanels.size(); ++i) {
      ...
      SearchStackOperator op = SearchStackOperator.parseOperator(mPanels.get(i).getLogical());

      search.add(UserSearchEntry.create(op,
          SearchCategoryService.getInstance().getSearchCategory(mPanels.get(i).getField()), mPanels.get(i).getText()));
    }
  ...
  public final Deque<SearchStackElementCategory> getSearchStack() throws Exception {
    ...
    Deque<SearchStackElementCategory> stack = SearchStackElementCategory.getSearchStack(search);
