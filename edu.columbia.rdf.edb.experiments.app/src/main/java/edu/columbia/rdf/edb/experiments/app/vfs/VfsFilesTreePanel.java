...
public class VfsFilesTreePanel extends ModernComponent {
	...
	/** The m tree. */
	private ModernTree<FileDescriptor> mTree = 
	...
	private class TreeEvents extends TreeEventAdapter {
		...
		/* (non-Javadoc)
		 * @see org.abh.common.ui.tree.TreeEventAdapter#treeNodeClicked(org.abh.common.ui.tree.ModernTreeEvent)
		 */
		@Override
	...
	public void loadDirs() throws ParseException, MalformedURLException, IOException {
		...
		/*
		 * TreeNode<FileDescriptor> root = 
				new TreeNode<FileDescriptor>("/");
		if (mExperiments.size() > 0) {
			for (Experiment experiment : mExperiments.keySet()) {
				FileDescriptor file = store.getExperimentFilesDir(experiment);

				TreeNode<FileDescriptor> expNode = 
						new TreeNode<FileDescriptor>(experiment.getName(), file);

				expNode.setIsParent(true);

				root.addChild(expNode);

				loadDirs(store, expNode);
			}
		} else {
		 */
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public void loadDirs(Repository store, TreeNode<FileDescriptor> node) throws ParseException, MalformedURLException, IOException {
	...
	private void setFiles() throws ParseException, MalformedURLException, IOException {
		...
		if (file != null) {
			...
			List<FileDescriptor> files = 
					repository.vfs().ls(mTree.getSelectedNode().getValue().getId());
